...
-- SELECT * FROM [ORDER]
-- SELECT * FROM [CLIENT]
-- select id_order,sum(price_position) as price_position from PRODUCT_LIST where price_position between 10000 and 80000 group by id_order
-- select id_order,sum(amount) as amount from PRODUCT_LIST group by id_order
-- insert into [CLIENT] ([name_client],[surname_client],[patronymic_client],[number_client],[IsDelete]) values ('name','surname','patron','45645654',0)
-- select max(price) as price from [dbo].[PRODUCT]
-- update [dbo].[PRODUCT] set name='name',male_female='Ц',price=546.00,IsDelete=0 where [id_product] = 1075
