    ...
    public partial class OrderForms : Form
    {
        ...
        private void DeleteOrderButton_Click(object sender, EventArgs e)
        {
            ...
            if (OrderGridViewer.SelectedRows.Count == 1)
            {
                ...
                {
                    string UpdateQuery = $"UPDATE [dbo].[ORDER] SET [IsDelete] = 1 WHERE [id_order] = {IdOrder}";
        ...
        private void OrderGridViewer_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                string SelectQuery = $"SELECT [id_client] FROM [dbo].CLIENT WHERE [name_client]='{Name}' AND [surname_client]='{Surname}' AND [patronymic_client]='{Patronymic}' AND [number_client]='{Number}'";
                SqlCommand command = new SqlCommand(SelectQuery, connect);
                SqlDataReader reader = command.ExecuteReader();
